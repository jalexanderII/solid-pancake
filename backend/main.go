...
func SetupApplicationRoutes(v1 fiber.Router) {
	...
	application.Post("/apply", Apply)
	application.Post("/:id/upload", Upload)
	// Application Request endpoints
	appReq := application.Group("/request")
	appReq.Get("/", GetApplications)
	appReq.Get("/:id", GetApplication)
	appReq.Delete("/:id", DeleteApplication)
	// Application Response endpoints
	appResp := application.Group("/response")
	appResp.Get("/:id", GetApplicationResponse)
	appResp.Delete("/:id", DeleteApplicationResponse)
}
...
var client = applicationpb.NewApplicationClient(getConn())
